...
static int param_log_decor = PJ_LOG_HAS_NEWLINE | PJ_LOG_HAS_TIME | 
...
extern int init_msg_logger_inv(pjsip_endpoint *endpt);
extern int msg_logger_set_enabled_inv(pj_bool_t enabled);
int test_inv_main(void)
{
    pj_status_t rc;
    const char *filename;
    ...
    /* Init logger module. */
    init_msg_logger_inv(endpt);
    msg_logger_set_enabled_inv(1);
