...
static uint32_t get_sockaddr_len(const vpk_sockaddr* addr) {
...
static void addr_cpy(vpk_sockaddr* dst, const vpk_sockaddr* src) {
	if(dst && src)
		bcopy(src, dst, sizeof(vpk_sockaddr));
}

static int addr_get_from_sock(int fd, vpk_sockaddr *addr)
{
	if (fd < 0 || !addr)
		return -1;
	else {
...
static int udp_send(int fd, const vpk_sockaddr* dest_addr, const char* buffer, int len)
{
	int rc = 0;
...
static int udp_recvfrom(int fd, vpk_sockaddr* orig_addr, const vpk_sockaddr *like_addr, char* buffer, int buf_size, int *ttl, int *tos, char *ecmsg, int flags, uint32_t *errcode)
{
	int len = 0;
...
static int set_sock_buf_size(int fd, int sz0)
{
	int sz;
	...
	if (sz < 1) {
		...
		LOG_D("Cannot set rcv sock size %d on fd %d\n", sz0, fd);
	}
	...
	if (sz < 1) {
		...
		LOG_D("Cannot set snd sock size %d on fd %d\n", sz0, fd);
	}
...
static int set_socket_ttl(int fd, int ttl)
{
...
int vpk_lan_server(int lport, int rport)
{
	...
	tv.tv_usec = 0;
	while (cnt--) {
		ret = select(server+1, &rd, NULL, NULL, &tv);
		if(ret == 0) // timeout
		{
			LOG_D("select time out!\n");
		}
		else if(ret == -1)  // failed
		{
			LOG_D("fail to select!\n");
		}
		else  // success
		{
			LOG_D("data is available!\n");
			udp_recvfrom(server, (vpk_sockaddr*)&remote_addr, (vpk_sockaddr*)&my_addr, recv_buff, MAXBUF, &ttl, &tos, cmsg, 0, NULL);
			LOG_D("remote_addr: %s:%d, my_addr: %s:%d\n", inet_ntoa(remote_addr.sin_addr), ntohs(remote_addr.sin_port) , inet_ntoa(my_addr.sin_addr), ntohs(my_addr.sin_port));
			LOG_D("TTL: %d\n", ttl);
		}
...
//		LOG_D("full: %s, pathname: %s", SAMPLE_ZLOG_FILE_PATH, tmp);
